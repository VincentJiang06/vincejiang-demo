...
const waveEnd = home.indexOf('</svg>', waveStart);
assert.ok(waveStart > -1 && waveEnd > waveStart, 'wave ribbons should render before the ambient svg closes');
const waveRibbons = home.slice(waveStart, waveEnd);
const echoPathCount = (waveRibbons.match(/<path d="/g) || []).length;
assert.equal(echoPathCount, 8, 'wave ribbons should stay as sparse low-density echoes near the main waves');
const ambientBg = home.slice(home.indexOf('<div class="ambient-bg"'), home.indexOf('</div>', home.indexOf('<div class="ambient-bg"')));
assert.doesNotMatch(ambientBg, /<rect /, 'ambient background should not include dot rectangles');
assert.match(home, /<animateTransform attributeName="transform" type="translate" values="-18 -3;18 4;-18 -3" dur="48s" repeatCount="indefinite"\/>/);
assert.match(waveRibbons, /<g class="wave-ribbons" opacity="\.56">/);
assert.doesNotMatch(waveRibbons, /values="0 -7;0 9;0 -7"/);
assert.equal((waveRibbons.match(/class="wave-echo"/g) || []).length, 4);
assert.match(waveRibbons, /stroke-dasharray="150 74 24 92"/);
assert.match(waveRibbons, /stroke-dasharray="210 96 36 128"/);
