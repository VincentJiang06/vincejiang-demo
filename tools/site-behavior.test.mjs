...
assert.match(home, /<pattern id="ambient-dot-grid" width="32" height="32" patternUnits="userSpaceOnUse" patternTransform="translate\(-8 -3\)">/);
assert.match(home, /<animateTransform attributeName="patternTransform" type="translate" values="-8 -3;8 5;-8 -3" dur="48s" repeatCount="indefinite"\/>/);
assert.match(home, /<circle cx="4" cy="4" r="1" fill="url\(#ambient-stroke\)" opacity="\.58"\/>/);
assert.match(home, /id="ambient-dot-wave-grid"/);
assert.match(home, /<circle cx="4" cy="4" r="2\.55" fill="url\(#ambient-stroke\)" opacity="\.86"\/>/);
assert.match(home, /<mask id="wave-dot-mask" maskUnits="userSpaceOnUse" x="-180" y="-100" width="1800" height="1180">/);
assert.match(home, /stroke-width="112" opacity="\.9"/);
assert.match(home, /<rect class="dot-matrix" x="-180" y="-100" width="1800" height="1180" fill="url\(#ambient-dot-grid\)" opacity="\.54"\/>/);
assert.match(home, /<rect class="dot-wave-matrix" x="-180" y="-100" width="1800" height="1180" fill="url\(#ambient-dot-wave-grid\)" mask="url\(#wave-dot-mask\)" opacity="\.82"\/>/);
...
assert.equal((ambientBg.match(/<rect /g) || []).length, 2, 'ambient background should use small dots plus larger wave-near dots');
assert.match(home, /<feDisplacementMap in="SourceGraphic" in2="noise" scale="9" xChannelSelector="R" yChannelSelector="G"\/>/);
assert.match(home, /<animateTransform attributeName="transform" type="translate" values="-20 -9;20 9;-20 -9" dur="52s" repeatCount="indefinite"\/>/);
