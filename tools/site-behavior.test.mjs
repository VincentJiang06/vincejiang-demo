...
assert.match(home, /<circle id="ambient-dot-core" cx="0" cy="0" r="1"\/>/);
...
assert.match(home, /<animateTransform attributeName="patternTransform" type="translate" values="-8 -4;8 6;-8 -4" dur="14s" repeatCount="indefinite"\/>/);
assert.match(home, /<g transform="translate\(4 4\)"><use href="#ambient-dot-core" fill="url\(#ambient-stroke\)" opacity="\.56"\/><\/g>/);
assert.match(home, /id="ambient-dot-mid-grid"/);
assert.match(home, /id="ambient-dot-near-grid"/);
assert.match(home, /<animateTransform attributeName="transform" type="scale" values="1;1\.7;1\.15;1\.55;1" keyTimes="0;\.22;\.46;\.72;1" dur="10s" repeatCount="indefinite"\/>/);
assert.match(home, /<animateTransform attributeName="transform" type="scale" values="1\.15;2\.45;1\.4;2\.75;1\.15" keyTimes="0;\.22;\.46;\.72;1" dur="10s" repeatCount="indefinite"\/>/);
...
assert.doesNotMatch(home, /id="wave-dot-grow"|feMorphology|filter="url\(#wave-dot-grow\)"/);
assert.match(home, /<mask id="wave-mid-mask" maskUnits="userSpaceOnUse" x="-180" y="-100" width="1800" height="1180">/);
assert.match(home, /<mask id="wave-near-mask" maskUnits="userSpaceOnUse" x="-180" y="-100" width="1800" height="1180">/);
assert.match(home, /stroke-width="150" opacity="\.5"/);
assert.match(home, /stroke-width="72" opacity="\.96"/);
assert.match(home, /class="dot-field wave-reactive mid" mask="url\(#wave-mid-mask\)" opacity="\.52"/);
assert.match(home, /class="dot-field wave-reactive near" mask="url\(#wave-near-mask\)" opacity="\.88"/);
...
assert.equal((ambientBg.match(/<rect /g) || []).length, 3, 'ambient background should use base dots plus two wave-bound circular scale layers');
assert.match(home, /<feDisplacementMap in="SourceGraphic" in2="noise" scale="5" xChannelSelector="R" yChannelSelector="G"\/>/);
assert.equal((ambientBg.match(/values="-34 -16;34 16;-34 -16" dur="10s"/g) || []).length, 5, 'wave masks, reactive dot patterns, and main wave should share the same faster motion');
...
assert.match(backgroundTest, /class="dot-field wave-reactive mid"/);
assert.match(backgroundTest, /class="dot-field wave-reactive near"/);
assert.match(backgroundTest, /id="wave-mid-mask"/);
assert.match(backgroundTest, /id="wave-near-mask"/);
assert.doesNotMatch(backgroundTest, /id="wave-dot-grow"|feMorphology|id="ambient-dot-wave-grid"|r="2\.55"|class="dot-wave-matrix"/);
