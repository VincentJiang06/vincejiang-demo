...
| `posts/<group>/<slug>.md` | `/research/<group>/<slug>/`(当 `<group>` 是 `collections.key`) |
| `posts/<group>/<slug>.en.md` | `/research/<group>/<slug>/en/`(英文子页,见 §4) |

**Research collection**(一组按顺序连排的论文):在 `site.config.json` 的 `collections` 加一条,`order` 列出 slug。该组自动获得落地页 `/research/<key>/`(编号清单)+ 每篇的「上一篇/下一篇」串联导航,即「多页面文件连排」。若另有 revision / 评述 / 复盘 collection,用 `revisionOf` 指回正文 collection,正文 collection 可用 `revisedBy` 指向它;`/research/` 会自动把正文和 Revision 成对展示。
...
3. 生成结果:collection 中文主页 `/research/…/<slug>/` + 英文子页 `/research/…/<slug>/en/`;**首页/Blog 索引/RSS 只收中文主页**(英文页不重复出现);两页互挂 `hreflang`(`x-default` 指中文),右上角药丸「中 / EN」互跳。
...
# 打开 http://localhost:8000/research/<group>/<slug>/;侧栏「打印 / 存 PDF」验 A4
```
