...
生成器页面的背景 = 固定层 `.bgfx` div + **静态渐变 SVG**(`assets/bg-{light,dark}.svg`,各 ~7KB,由
`tools/gen-bg.mjs` 生成)。形态 = **曲线状散开的线性渐变**:4 条贯穿画布的大 S 形贝塞尔色带,每条用
linearGradient 沿走向着色(2~3 个低饱和色相过渡),边缘靠「多层同形描边羽化」散开 —— 同一条曲线画 11 层,
stroke-width 由粗到细、opacity 由淡到浓。亮=白底粉彩带,暗=近黑底冷调带。
**纯静态:零脚本、零动画、零滤镜**(不用 feGaussianBlur —— 大面积模糊在低端机上是真开销,多层描边等价且零滤镜)。
