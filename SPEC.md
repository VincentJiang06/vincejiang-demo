...
### 3.3 流场星尘 + 流星干涉背景(2026-08-04)

生成器页面的背景 = 固定层 `<canvas class="bgfx">` + base.html 内联脚本:点状星尘沿层叠正弦伪噪声流场缓漂,
短彗尾由半透明罩层快衰减画出(亮罩 .12/暗罩 .10,**不留长残迹**;明暗两态的点色分别调校);**每 4~9s 一颗流星**
斜划而过(亮头 + 16 段渐隐尾,22% 概率双流星),途经 140px 内的星尘被冲开、~1.5s 指数衰减后回归流场(干涉)。
灰点为主 + 38% 低饱和彩点(色相集与色条同语言)。**性能红线:星尘按面积上限 150 + 流星 ≤2;30fps 节流;
DPR≤1.5;页隐藏即停**。`prefers-reduced-motion` / 无 JS / 无 canvas 退静态海浪 SVG(gen-waves.mjs 产物)。
论文页与各 demo 不吃 site.css,不受影响。背景裁决链:纯色(07)→静态海浪→粒子堆砌(被否)→流场丝线(被否
「乱/拖尾/无趣」)→**星尘+流星干涉(现行)**;行为测试断言随裁决走。

另:全站交互规范 —— **hover 一律不位移**(2026-08-04 用户裁决,translateY 全部移除),用色相描边光环 +
标题变色指示;标签 chip 统一 flat 化(.tag inline-flex,无玻璃特效),星标「主打」的 ★ 由 CSS ::before 画
(emoji 排版不稳)。
