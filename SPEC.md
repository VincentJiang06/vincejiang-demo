...
生成器页面的背景 = 固定层 `.bgfx` div + **静态渐变 SVG**(`assets/bg-{light,dark}.svg`,各 ~12KB,由
`tools/gen-bg.mjs` 生成)。形态 = **等高线路径的曲线色带**:
- **路径**沿用等高线图那套 —— 种子化高程场(高斯峰谷+缓和正弦)→ marching squares 取等值线 → 链段成折线 →
  抽稀 → Catmull-Rom 转贝塞尔平滑;从多档等值线里挑「舒展的开放曲线」(span/len > ~0.34)且质心分散的 5 条;
- **着色**每条用 linearGradient 沿走向过渡 2~3 个低饱和色相;
- **散开**靠多层同形描边羽化(11 层,width 粗→细、opacity 淡→浓),路径只在 `<defs>` 存一份、各层用 `<use>`
  引用(等值线折点多,11 层重复会把文件撑到几十 KB)。
**纯静态:零脚本、零动画、零滤镜**(不用 feGaussianBlur —— 大面积模糊在低端机上是真开销)。

调校经验(踩过的坑,改之前先看):
1. 别加全幅 wash 罩层 —— 会把所有色相压成一片脏灰;
2. 亮色要「高明度 + 够高饱和」(hsl ~74% 88%),饱和度低了在白底上只剩灰;
3. 暗色底 `#07070c`、色带亮度 ~16%,否则死黑没层次;
4. **暗色下暖色(橙/粉)会发浑** —— 用 `dk` 权重单独压低或归零,暗色只留冷调;
5. 羽化层数别少于 ~9,层数少了能看出一圈圈同心边界(带状条纹);
6. **等值线要挑舒展的开放曲线** —— 闭合/紧绕的渲成色带像涂鸦圈,背景要的是「掠过」感;
7. **色带别太宽**(超画布高 ~20% 时弯曲被羽化糊成水平条),带宽 110~190;
8. 背景要克制 —— 单层 alpha 系数压在 ~0.05(亮)/0.06(暗),浓了会抢正文。

改构图/配色 → 改 `gen-bg.mjs` 重跑 `node tools/gen-bg.mjs`,产物要提交。论文页与各 demo 不吃 site.css。
背景裁决链:纯色→静态海浪→粒子堆砌(否)→流场丝线(否)→星尘流星(否)→等高线(否,"太丑")→点状晕团渐变(否)
→人工 S 弯色带(否)→**等高线路径色带(终版)**;
