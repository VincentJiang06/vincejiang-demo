...
`reactor-study/`(产物,对外) ← 由 `reactor-study-src/site`(零框架静态生成器)编译:

```bash
cd reactor-study-src/site
BASE=/reactor-study SITE=https://vincejiang.com/reactor-study node build.mjs
rsync -a --delete dist/ ../../reactor-study/
```

`BASE` 给所有内部链接加前缀(含 `<html data-base>`,供前端运行时拼 URL),`SITE` 定 canonical/sitemap;构建末尾会给 `theme/*.css` 与 `modules/*.js` 的引用打 `?v=<内容哈希>`。
