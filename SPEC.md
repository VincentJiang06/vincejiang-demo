...
### 3.3 流场粒子背景(2026-08-04)

生成器页面的背景 = 固定层 `<canvas class="bgfx">` + base.html 内联脚本:粒子沿**层叠正弦伪噪声流场**
(空间平滑、随时间缓慢换向)漂移,拖出丝绸状细流痕,整页织成抽象流动纹理;渐隐靠每帧一层半透明底色罩
(无逐帧清屏,暗色罩 .018/亮色 .06,暗色描边更亮更浓补偿黑底吃细节);灰线为主 + 40% 低饱和彩线(色相集
205/268/24/215/165/330/242,与色条同语言);init/换主题时预热 150 帧,开屏即有织好的纹理。
**性能红线:粒子按面积自适应、上限 260;30fps 节流;DPR≤1.5;页隐藏即停**。`prefers-reduced-motion` /
无 JS / 无 canvas 一律退回静态海浪 SVG(`assets/bg-waves-{light,dark}.svg`,`tools/gen-waves.mjs` 生成)。
论文页与各 demo 不吃 site.css,不受影响。背景裁决链:纯色(07)→静态海浪(08-03)→粒子堆砌(08-04 上午,被否
「粒子太少没意思」)→**流场粒子(现行)**;行为测试断言随裁决走。
