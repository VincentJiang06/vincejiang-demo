...
import { mount, controls, slider, readout, presets, stepper, miniTable } from "/modules/mod-kit.js?v=49b358d492";
...
const CASES = [
  { k: "01", t: "US News 法学院", vals: [10, 10, 9, 8, 10, 7, 6],
    line: "近乎垄断的年度全序名次，连着生源与捐款：教科书级的反应性极端案例（R01 的主角）。",
    why: ["USN 一家独大数十年，法学院无榜可挑", "名次直接牵动生源、捐款、院长的饭碗", "使命各异的学校被压成第 1 到第 199 名",
      "一年一放榜，整个招生周期跟着它转", "名次是相对的：对手涨你就掉", "冲分奖学金、就业统计口径都是现成通道", "院长们雇顾问逆向工程公式"] },
  { k: "02", t: "商学院（五榜并存）", vals: [3, 7, 8, 6, 8, 7, 6],
    line: "五家榜单方法各异，垄断度这格掉下来，整体强度低一截——多重排名互相稀释。",
    why: ["BW/USN/FT/Economist/WSJ 五家并存，可挑着讲", "利害仍高：学费与生源都在赌桌上", "同样压成名次，粗暴度不减",
      "各家节律不一，张榜的声浪被稀释", "同样是零和名次", "通道与法学院类似", "同样会逆向工程，但火力分散到五个公式"] },
  { k: "03", t: "高考", vals: [9, 10, 9, 3, 10, 6, 8],
    line: "利害与零和拉满，但一年一次的低节律和严格监考压住了部分扭曲——扭曲改道去了应试训练。",
    why: ["一考定位次，几乎是唯一通道", "直接决定人生走向，利害拉满", "总分一个数，压掉所有侧面",
      "三年备考只为一次放榜：节律低，但赌注极重", "按位次录取，标准的零和", "题型可押、套路可练：应试即绕行", "整个应试产业都在逆向工程考纲"] },
  { k: "04", t: "Chatbot Arena（AI 排行榜）", vals: [8, 9, 9, 10, 9, 8, 10],
    line: "全天候更新 + 被测者是会读规则的模型与厂商：反身能力被推到人类场景没有的档位（Y11 细讲）。",
    why: ["一度是社区默认的「唯一」大模型榜", "发布会、融资、招聘都盯着它", "万千能力压成一个 Elo 分（相对胜率折算的棋类等级分）",
      "全天候滚动更新：永恒监考", "Elo 本身就是零和的相对分", "私测多变体、只发布最高分等通道", "厂商与模型都能认出考场、读懂规则"] },
  { k: "05", t: "好的内部诊断指标", vals: [2, 2, 3, 3, 1, 3, 4],
    line: "不挂利害、不排名、多维仪表盘：测量安安静静地当诊断用——这是低反应性长什么样。",
    why: ["只是团队内部多把尺子之一", "不连 KPI，不连发布决策", "保留多维仪表盘，不排名",
      "按需查看，没有固定张榜日", "看绝对读数，不比相对名次", "没人有动机绕它", "没有公开公式可供逆向"] }
];
...
mount("seven-levers", (body, fig, { config }) => {
  ...
  const pb = presets(body, {
    label: "一键载入真实案例（七根杠杆拨到对应档位）", items: CASES,
    onselect: (c, i) => {
      caseIdx = i;
      applying = true;
  ...
  caseLine.textContent = "还没选案例——点上面一键载入，或直接拖滑块自定义。";
  ...
  const sliders = LEVERS.map(([name, desc], i) => {
    ...
    s.on(() => {
      ...
      if (!applying && caseIdx != null) {          // 手动拖过就不再是那个案例
        ...
        caseLine.textContent = "自定义档位（不对应任何预设案例）。";
        explain(stepIdx);
      }
    });
  ...
  const mt = miniTable(body, {
    cols: ["杠杆", "档位", "贡献分", "一句白话"], rows: [],
    note: "七格贡献分相加 ≈ 总指数（0–100）。哪格高，失真就先从哪格开始——这也是 C03 降压时的动手顺序。"
  });
  ...
  const st = stepper(body, {
    count: LEVERS.length, prevLabel: "上一根", nextLabel: "下一根",
  ...
  function explain(i) {
    ...
    panel.innerHTML = `<span class="label">杠杆 ${i + 1}/7 · ${LEVERS[i][0]}</span>
      <p><strong>它问的是：</strong>${LEVERS[i][2]}。</p>
      <p><strong>为什么它是根杠杆：</strong>${LEVERS[i][3]}</p>
      ${c ? `<p class="case-line"><strong>${c.t}</strong> 这格打 <strong>${c.vals[i]}/10</strong>：${c.why[i]}</p>`
          : `<p class="case-line">在上面载入一个真实案例，这里会解释它这一格为什么打这个分。</p>`}`;
  }
