...
import { progress } from "/modules/boot.js?v=r2";
import { TREE } from "/modules/tree-data.js?v=r2";
...
function init(vp, mode) {
  ...
  function home() {
    ...
    // 但比例不低于 0.5 保住芯片可读；放不下时靠约束保证最左芯片不藏进浮层后面
    scale = Math.max(0.5, Math.min(0.66, (r.width - left - 48) / W));
  ...
  function fitBranch(branch) {
    const sel = branch === "green" ? ["green", "converge"] : [branch];
    const ns = nodes.filter(n => sel.includes(n.branch));
    if (!ns.length) return;
    const pad = 120;
