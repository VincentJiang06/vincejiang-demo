...
import { mount, canvas, readout, cssvar, stepper } from "/modules/mod-kit.js?v=49b358d492";
...
mount("the-loop", (body, fig) => {
  ...
  hint.textContent = "把光标移到点阵上 = 观察它。看看会发生什么。（下面有三步小导览，跟着走就行）";
  ...
  function renderNote(i) {
    if (i === 0) note.innerHTML = `<span class="label">第 1 步 / 3 · 观察前的世界</span>
      <p>这 220 个点各有一个「真实值」——可以想成 220 个学生各自真实的水平。现在还没有人看它们，所以屏幕上的「测量值」和「真实值」完全一致：上面读数里的「相关」是 1.00。（相关＝两组数字步调一致的程度，1 是完全同步，0 是各走各的。）</p>
      <p>什么都还没发生。这就是尺子举起来之前的世界。</p>`;
    else if (i === 1) note.innerHTML = `<span class="label">第 2 步 / 3 · 你开始观察</span>
      <p>把光标移进点阵——那圈光晕就是你的「注视」。被罩住的点算「被你观察」：它会变亮、读数往上飘一点，像学生知道老师在看，答题姿势立刻变了。</p>
      <p>多扫几圈，同时盯着上面两个数字：「已观察」在涨，「测量↔真实相关」在掉。</p>`;
    else note.innerHTML = `<span class="label">第 3 步 / 3 · 相关性崩塌</span>
      <p>你刚才亲手做了一遍这门课要讲的事：<strong>观察本身改变了被观察的东西</strong>。被你看过的点，测量值被抬高了，真实值几乎没动——「读数」和「真实」开始各说各话，相关从 1.00 掉到了 ${lastCorr.toFixed(2)}（已观察 ${(lastObs * 100).toFixed(0)}%，想看它继续掉就再多扫几圈）。</p>
      <p><strong>这说明什么：</strong>考试、医院排名、AI 模型评测，都是这个小把戏的放大版——尺子一举起来，被量的对象就开始朝尺子表演，你测到的就不再是原来那个东西。这个现象叫「反应性」，这门课后面的每一课，讲的都是它的一个变种。点「↺ 重置」可以回到无人观察的世界再来一遍。</p>`;
  }
  const st = stepper(body, {
    count: 3, prevLabel: "上一步", nextLabel: "下一步",
