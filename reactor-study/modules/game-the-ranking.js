...
import { mount, canvas, controls, slider, add, readout, legend, cssvar, presets, miniTable } from "/modules/mod-kit.js?v=49b358d492";
...
mount("game-the-ranking", (body, fig) => {
  ...
  predictGate(body, {
    q: "上任先押一注：如果把预算几乎全砸在操纵指标上，「名次」和「真实办学质量」谁先有明显反应？",
    options: [
      { t: "名次先动：一两年就蹿上去，真实质量的亏空要好几年才显形", ok: true },
      { t: "真实质量先动：立刻塌下去，名次反应要慢半拍" },
      { t: "两个同时、同速变化" }
    ],
    reveal: `榜单的公式是：榜面分 = 真实质量 + 1.2×指标投入 + 噪声。指标投入是<strong>当年到账</strong>的，名次立刻反应；而真实质量每年只挪一小步（增长 0.02×办学投入），亏空要好几年才看得出来。下面的逐年对账表能亲手对出这个时间差：「名次」一列先变，「真实质量」一列慢慢掉。`,
    plain: "操纵的回报是即时的，代价是慢性的。这正是每个院长都忍不住动手、军备竞赛必然点燃的原因——等亏空显形时，全场都在操纵，名次早已退不回去。",
    actHint: "把年份推进几年，就能对答案"
  }, buildSim);
  ...
  function buildSim(host, act) {
    ...
    function reset() {
      ...
      prevRank = null;
      t = 0; history = { rank: [], quality: [], gaming: [], mine: [] };
    ...
    /* 三档人设：一键把「押指标比例」拨到典型档位。中途改弦更张也行——
       模拟不重置，正好观察策略切换的滞后效果。 */
    const PERSONA = [
      { k: "A", t: "老实办学", g: 5, note: "预算 95% 投真实办学，只留 5% 应付指标" },
      { k: "B", t: "精明经营", g: 35, note: "拿三分之一预算喂指标，其余照常办学" },
      { k: "C", t: "全押指标", g: 85, note: "预算几乎全部砸向指标，办学靠惯性" }
    ];
    let applying = false;
    const pb = presets(ctrl, {
      label: "一键换人设（中途改也行）", items: PERSONA,
    ...
    const mt = miniTable(host, {
      cols: ["年", "你押指标", "名次", "真实质量", "全场平均押注"], rows: [],
      note: "逐年对账（最近 10 年）。对着看：「你押指标」一变，「名次」当年就反应；「真实质量」要过几年才显形。"
    });
