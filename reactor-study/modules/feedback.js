...
import { mount, controls, slider, add, readout, legend, scoped, compareCanvas, presets } from "/modules/mod-kit.js?v=49b358d492";

const SCEN = [
  {
    k: "01", t: "课堂期望", pair: "学生",
    intro: "两个学生，真实水平完全相同。唯一的区别：开学时老师被（随机地！）告知其中一位是「潜力股」。",
    chan: "这里的传动装置（把信念变成现实的那条通道）是：老师的期望 → 更多提问机会、更多耐心、更高容错 → 真实成绩。"
  },
  {
    k: "02", t: "信贷评分", pair: "借款人",
    intro: "两个人，还款能力完全相同。唯一的区别：一个的初始信用分被算高了一点点。",
    chan: "这里的传动装置是：分数 → 更低利率、更高额度 → 更宽松的财务处境 → 真实还款能力。"
  },
  {
    k: "03", t: "排名", pair: "学校",
    intro: "两所学校，真实质量完全相同。唯一的区别：首次排名里，一所因为测量噪声排在了前面。",
    chan: "这里的传动装置是：名次 → 申请者与捐款的流向 → 办学资源 → 真实质量。"
  }
];
...
mount("feedback", (body, fig, { config }) => {
  ...
  const pb = presets(body, {
    label: "换一个场景（机制是同一台）", items: SCEN, value: 0,
    onselect: it => { scen = it; syncIntro(); draw(); }
  });

  const ctrl = controls(body);
  add(ctrl, slider(body, {
    label: "右边世界的反馈强度（预言→资源→现实）",
    min: 0, max: 100, step: 5, value: 80, fmt: v => (v / 100).toFixed(2)
  }).on(v => { fb = v / 100; draw(); }));

  const cc = compareCanvas(body, {
    a: "反馈 = 0 的世界（预言只是说说）",
    b: "反馈 = 0.80 的世界（预言带来资源）",
    h: 260
  });
  const out = readout(body, "");
  legend(body, [
    { c: "var(--accent)", t: "被看好的一方（真实能力）" },
    { c: "var(--ink-300)", t: "被看衰的一方（真实能力）" }
  ]);

  function syncIntro() {
    intro.textContent = scen.intro + " 左边世界的反馈被剪断（=0）；右边世界预言换得动资源。拖滑块调右边的反馈强度。";
  }
  syncIntro();
  ...
  function paint(cell, S, hi, lo) {
    ...
    x.fillText("时间 →", pad + gw - 40, h - 8);
    x.fillText("末期真实差距 " + (S.A.at(-1) - S.B.at(-1)).toFixed(3), pad + 2, pad - 8);
  }
  ...
  function draw() {
    ...
    paint(cc.a, ra, hi, lo);
    paint(cc.b, rb, hi, lo);
    const bt = `反馈 = ${fb.toFixed(2)} 的世界（预言带来资源）`;
    if (cc.b.label.textContent !== bt) cc.b.label.textContent = bt;   // 值没变就不动 DOM
    const gap = rb.A.at(-1) - rb.B.at(-1);
    const html = `左边世界末期差距 <span class="big">${(ra.A.at(-1) - ra.B.at(-1)).toFixed(3)}</span>
      &nbsp;·&nbsp; 右边世界末期差距 <span class="big">${gap.toFixed(3)}</span>（两边初始差距都是 0）
      <div style="font-size:.8rem;margin-top:6px;opacity:.9">${fb > 0.05
        ? `同样的两个${scen.pair}，右边只多了一条传动带。${scen.chan} <strong>这说明什么：</strong>差距不是天赋造出来的，是传动带造出来的——剪断传动带（反馈=0），预言就只是空话；传动带越粗，一点噪声被坐实得越快、越大。这就是 Merton 的自我实现闭环。`
        : "反馈=0 时两个世界一模一样：预言无法自我实现，它只是空话。把滑块推上去，看右边世界怎么被推开。"}</div>`;
