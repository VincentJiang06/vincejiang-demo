...
目录
  ...
  curves/<型号>/*.csv      原始频响曲线（frequency,raw 两列），共 339 条（同声学型号与兄弟型号共用的文件各存一份，去重后 300 份不同测量）；178 款里有 24 款有本型号自己的 B&K 5128 曲线
  ...
  data/curves_manifest.csv 每条曲线的来源、台架（rig / rig_key）、身份 identity（本型号 exact / 同声学 same-acoustics / 代理 sibling）、类型 kind（常规 / 换垫 / 佩戴位置 / 代理…）、is_5128（台架是否 B&K 5128，与身份无关）、own_measurement（= identity 为 exact 或 same-acoustics 且 kind 是单次实测：合成曲线和其他版本即使 identity=exact 也为 False）、label（页面上的曲线标签）、所用参考 ref；scoring_grade = 按复核标准够格作为打分依据，agent_used_when_scoring = 研究 agent 当时打分时参考过（两者不是一回事）、与参考曲线的偏差、原始 URL、复核备注。同声学型号与其兄弟型号共用同一份曲线文件，各计一条
...
口径
  ...
  性价比 = 综合分（音质与听歌均值）减去同价位趋势线预期；趋势线只用 board_ok = True 的型号拟合。
  board_ok = True 表示存档里有本型号（measured）或声学相同的兄弟型号（measured-twin）的实测数值曲线；
  页面上的三张榜单（听歌 / 挑错 / 性价比）只收这些型号，其余型号分数打得保守，不进榜，但仍在坐标图、数据表和这两个数据文件里。
