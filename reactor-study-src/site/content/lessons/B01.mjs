...
<p>这句话流传极广，只有一个问题：不是 Goodhart 说的。</p>` },
    ...
    { t: "prose", html: `
<p>真实的故事比传说更有层次。1975 年，经济学家 Charles Goodhart 在给英格兰银行做货币政策顾问，观察到英国货币目标制的怪事：政府一旦盯住某个货币总量（£M3）调控，这个总量和经济之间原本稳定的关系就崩了。目标定在 7% 到 11%，实际冲到 18% 到 19%，同期通胀却在下降。他把观察写成一句半开玩笑的话，塞进会议论文的脚注。</p>
<p>考证还揭出两层被流行叙事抹平的结构。第一层，家谱要前推：Ridgway 1956 年已把「指标一成考核就变形」写成系统综述，比任何人名定律都早，这件事在被冠名之前就是组织社会学的田野常识。第二层，优先权要收紧：Chrystal 与 Mizen 2003 年的权威梳理逐字裁定，「若 Goodhart 定律与 Lucas 批判等价，Lucas 几乎肯定先说」；两者是分工不同的孪生命题，Lucas 改变了宏观计量方法论，Goodhart 改变了货币政策设计。「几条定律同时独立发现」的说法，准确版本是一个横跨组织社会学（1956）、社会科学（1969–79）、经济学（1975–76）的层累过程，Goodhart 因命名之功获得最大身后名。</p>` },
...
<p><strong>这也是 reward hacking 与 eval 退化的源头命题，而且 2022 年之后它从格言变成了定理。</strong>Skalse 等人证明，除平凡情形外，任何一对不完全对齐的奖励几乎必然可被博弈；Gao 等人测出了过优化的标度律。你的优化器（RL，或你手动挑 checkpoint 的那只手）就是那个被 KPI 驱动的工程师：它不怀恶意，只是忠实地把读数推高，而推高读数最省力的路径几乎从不是「变得更强」。<code>B08</code> 把「崩塌」拆成四种机制，黄色分支是它在梯度下降里的重演。</p>` },

    { t: "prose", html: `
<p>留一个问题：下次有人拿一句名言、一个指标或一条干净结论说服你，做两件事，查它的出处，问它被优化了多久。名言和指标一样，越是好用、被反复使用，内核越可能已经在使用中被悄悄换掉。</p>` },

    { t: "sources", items: [
      `Ridgway, V. F. (1956). "Dysfunctional Consequences of Performance Measurements." <em>ASQ</em> 1(2):240–247（家族最早一手源）。`,
      `Goodhart, C. (1975)；精炼句锚定 <em>Monetary Theory and Practice</em> (1984, p.96)。`,
      `Strathern, M. (1997). <em>European Review</em> 5(3):305–321 (p.308)；Hoskin, K. (1996) 命名。`,
      `Chrystal, K. A. & Mizen, P. D. (2003). "Goodhart's Law: Its Origins, Meaning and Implications for Monetary Policy"（Lucas 优先权裁定）。`,
      `完整考证见 <code>research/03a</code> 与 <code>research/deep/D2</code> §1–2。`
    ] }
  ]
};
