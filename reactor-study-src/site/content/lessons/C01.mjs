export default {
  ...
  blocks: [
    { t: "prose", html: `
<p>一个做模型评测的人皱着眉说，我的 eval 有 performativity（操演性）问题。你追问，他描述的其实是：模型被反复测，慢慢长成了这套题的形状。那不是操演性，那是反应性。用错词看着无伤大雅，可下一步就麻烦了，因为不同的词指向完全不同的对策，抓错病名就会开错药。</p>
<p>五个总被当成同义词的近邻要分开：reactivity（反应性）、performativity（操演性）、looping effects（循环效应）、legibility（可读性）、audit culture（审计文化）。它们确实亲缘，共享一个内核，但每一个都比其余多说了一点别的。分清它们，靠问三个问题：谁在回望、朝哪个方向使劲、以及它相对反应性多讲了什么。</p>` },
...
<p>D6 报告把这个不变式写成了一句精确的话：<strong>当你用一个低维、带噪、会被回应的指标，去优化或挑选一个高维、会回应的系统，你收敛到的不是系统的真优，而是指标的不动点、博弈的均衡、选择偏差的上序统计量、或退化模型的同态像；偏离的量随优化压力单调上升。</strong>契约论、优化不动点、Stackelberg 博弈、历史社会学、控制论、统计学，六种语言各自把它证过一遍。不必现在就读懂每个词，只要记住：这五个近邻不是散落的比喻，背后站着同一条可以被证明的规律。</p>
