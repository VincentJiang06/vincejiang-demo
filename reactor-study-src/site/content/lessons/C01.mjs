...
<p><strong>这张地图的用处在诊断阶段。</strong>你的 eval 出问题，可能是反应性（模型因被测而变）、操演性（这个 eval 在重塑整个模型生态）、或可读性（rubric 预先决定了什么可见），三者对应三种完全不同的干预。此外还要加一条独立于反应性的通道：选择偏差（<code>B13</code>）。就算被测者完全诚实，光是「从一堆里挑最高分」这个动作本身，也会让榜首虚高。所以诊断的第一步不是急着找对策，是先认准它属于哪一个机制家族，别拿操演性的药去治反应性的病。</p>` },
