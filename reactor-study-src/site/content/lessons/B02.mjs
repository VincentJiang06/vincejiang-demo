...
<p>翻译成白话：一个量化指标越是被用来做重要决策，它受到的腐蚀压力就越大，而且它越会扭曲、腐蚀它本来要监测的那个过程。注意最后半句：受害的不只是数字，还有数字背后的事。这句话出自小节标题就叫「量化指标的腐蚀效应」的一节，Campbell 用的字眼是复数的 pessimistic laws：他给出的是一组悲观定律，后人只记住了这一条。</p>` },
    ...
    { t: "prose", html: `
<p>Campbell 1979 年论文里的例子都带着案卷气，先讲警察局的。</p>
<p>警队按「清案率」考核，也就是破案占报案的比例。这个指标产生了双重腐蚀。报案端：案件被不记录、延迟记录，或降格成较轻的类别，报案少了，分母就小了。Campbell 点名尼克松的严打，说其主要效果是腐蚀了犯罪率指标（Seidman 与 Couzens 1972 的研究）。破案端更荒诞：辩诉交易里，被捕的窃贼多认几桩没头绪的悬案，换取轻判，用 Campbell 引的话说，「他在帮警察提高清案率」；而 Skolnick 1966 年的研究指出，其中不少人认下的是自己没犯的罪。清案率越好看，档案离真相越远。</p>
<p>再讲学校的。Texarkana 的绩效合同案（Stake 1971 记录）：教育承包商按学生成绩涨幅收钱，于是直接把期末考题拿来教。分数涨了，钱付了，学生什么也没多学。随后才是 Campbell 最常被引的那段话：在以一般能力为目标的正常教学下，成绩测验可能是不错的学业指标；一旦分数成为教学过程的目标，它既丧失指标价值，又以不良方式扭曲教育过程。</p>
<p>他的例证还跨了国界和战场：苏联的产量指标文献（Granick 1954、Berliner 1957），以及越战的歼敌数，即 body count（按尸体数汇报战果的考核法）。歼敌数一旦成了考核部队的硬指标，部队就有了往上凑数、甚至滥杀充数的压力；美莱村惨案，在他笔下正是这股压力被推到极端的战场形态，这条线在 <code>B05</code>（McNamara 谬误）展开。</p>` },
...
<p><strong>定律的语法本身就是机制：它说的不是「用了指标就腐蚀」，而是「越用于决策、越受腐蚀」。</strong>腐蚀压力随利害程度单调上升，是一条剂量反应曲线：低利害时指标大体诚实，因为没人有理由花成本去扭曲它；利害每加一档，扭曲的期望收益就抬高一档，直到操纵指标比改善实事更划算。所以同一份成绩测验，用于诊断时是不错的指标，用于发钱和摘乌纱帽时立刻变质。腐蚀的燃料不是测量，是挂在测量上的利害。</p>` },
    ...
    { t: "prose", html: `
<p>这个定律最容易被读错的地方，是把它当成反对量化的檄文。Campbell 的学术身份恰好相反：他与 Stanley 合著的实验与准实验设计教材，是整个项目评估领域的奠基文本；他 1969 年的《Reforms as Experiments》号召把社会改革当成实验来做、用严格测量指导政策。他是「实验社会」最有力的旗手。</p>
<p>张力在于，同一篇 1979 年论文里，他既号召大规模量化评估，又断言量化指标必被腐蚀。他的调和方案不是弃测，而是制度设计：引入外部评估者；让工会等利益相关方充当看门狗；多个指标三角互证；尽量让指标远离直接奖惩回路。他的原话是，我们应当研究腐败得以被揭露的社会过程，并尝试设计吸纳这些特征的社会系统。定律因此是实验社会的内置警戒条款，不是对它的否定。这条思路后来长成了整条防御分支：<code>G04</code> 讲多指标制衡，<code>G06</code> 讲制度层防御。</p>
...
<p><strong>你的 eval 不只是会失准，它会腐蚀你的开发过程。</strong>这是 Campbell 比 Goodhart 更进一层的地方。当一个 benchmark 分数（benchmark：给模型统一出题、统一打分的公开题库）成为团队的高利害目标，它不只是分数虚高：研究方向、数据配比、模型选择的整个过程都会被它扭曲，迎合可测的那一小块，牺牲不可测却更重要的能力。测量腐蚀的不是数字，是造数字的人的判断力。2026 年 OpenAI 弃用 SWE-bench Verified 的复盘（<code>Y03</code>）就是这条定律的工业级现场。</p>` },
...
<p><strong>一句话带走：</strong>高利害指标的杀伤半径不止数字本身，它会把产生数字的那件事一起改造掉。</p>` },
