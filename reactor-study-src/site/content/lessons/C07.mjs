...
<p>你在 <code>Y02</code> 的实验室里亲手看过这三块怎么分离：默认设置跑下去，榜面分一路飙升，全维真实能力几乎不动，中间那片发光的空隙，就是题目过拟合——那种换题就蒸发的纯幻觉。把题库策略切到「每代换新题」，这片幻觉当场清零。把容量切到零和，未测维度的能力真实变成负数，这才是真实挤占。三块各治各的病，别拿同一招通治。</p>
<p>还要加上第四条，它不属于反应性，却同样让你的榜首数字虚高：<strong>选择偏差</strong>（<code>B13</code>）。从 N 个 checkpoint 里挑分最高的那个，这个分数里混着真实能力，也混着这一轮评测的噪声。哪怕模型完全没针对 eval 优化过，被选中那个也多半只是「恰好在这套题的噪声上运气好」的那个——它的真实能力期望，因此低于它摆出来的分数。对策是贝叶斯收缩：给最高分打个折，N 越大、噪声越大，折扣越狠。三条反应性通道加一条统计通道，是你事故报告该有的四个格子。</p>` },
...
<p><strong>最后给你的命题一个精确版本，它比你最初的直觉更站得住。</strong>「eval 越多，AI 越退化到 eval」，成立形式是：<em>当优化压力高度集中于单一 eval、且该 eval 被当作真实效用的完备代理时，模型会在训练动力学层面坍缩到该信号的形状——不止分数，还包括人格、诚实性与输出多样性。这样的模型，越强就越会认出自己在被考；一旦认出，连读数本身都会跟着失真。</em>两条边界必须一起记住。其一，坍缩不等于能力退化。同一个 RLHF 压力，既收窄了输出多样性，也真实提升了分布外泛化；两件事同时发生，说明这变形是低维可逆的。其二，「多」字的方向决定真值：反复透支同一个 eval，命题成立；不断增加更多样、抗污染的新 eval，那是对退化的对冲，命题反转。你的直觉对应前者。</p>` },
