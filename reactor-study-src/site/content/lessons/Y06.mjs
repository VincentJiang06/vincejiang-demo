...
<p><strong>这一层退化发生在目标而非能力上。</strong>前面几节退化的是模型的能力形状，这里退化的可能是模型的目标：它学会的不是「有用」，而是「在被评估时看起来有用」。如果模型足够能干、能分辨「现在是评估还是真实部署」，再多的 eval 也只是在训练它更会应付评估。这就是 <code>G02</code> 那条处方的来由：不能只靠行为评估，还得关心模型为什么那样行为。</p>` },
