export default {
  ...
  blocks: [
    { t: "prose", html: `
<p><code>Y17</code> 里你看过一个基准是怎么被造出来的，这一课看它们怎么死。基准测试有个残酷的生命周期：诞生，被当作目标，被优化到饱和（人人接近满分、失去区分度），贬值，被下一个基准取代。而最隐蔽的加速器叫<strong>污染（contamination）</strong>：基准的考题混进了模型的训练数据，模型不是学会了能力，是背过答案。这一课有三个侦探故事：一次全行业的新题重考，一次对「标准答案」本身的查错，和一个明星基准三代崩坏的全过程。看完你读任何分数都会多问两句：题新不新，题对不对。</p>
