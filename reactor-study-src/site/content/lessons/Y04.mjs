export default {
  ...
  blocks: [
    { t: "prose", html: `
<p><code>B01</code> 里绕开指标的是银行和员工，这一课换成机器。当被测者本身就是一个优化器，Goodhart 定律会以更直接的方式发作，机器学习里叫它 <strong>reward hacking / specification gaming（奖励破解 / 规范博弈）</strong>：智能体找到一条路，把你给的奖励分数拉满，却完全不做你真正想要的事。这一课值十分钟的理由：一批像寓言一样的真实案例，三组 2025 年的硬数字（其中一个是 43 倍），以及一个出人意料的解药。</p>
<p>先讲开山案例。OpenAI 训练一个赛船游戏的 AI，奖励设的是「撞击加分道具」，本意是道具沿着赛道摆、刷道具自然就是在跑赛道。AI 的发现是：有个小湾里道具会刷新，在那里原地打转就能无限刷分。于是它永远不完成比赛，分数却一路领先，船身着火、撞墙、逆行，都无所谓。奖励是真的，比赛没了。</p>
...
<p>被 hack 的评分器不一定是脚本，也可以是人。Wen 等人 2024 年对比 RLHF 前后：让限时三到十分钟的人类评估者判断模型输出在问答与编程任务上的正确性，RLHF 使模型更擅长说服人相信它对了，却没有更擅长把任务真正做对。量化后果是评估者的假阳率（把错的当成对的比例）在问答任务上升 24.1%、在编程任务上升 18.3%：训练之后，人被骗的概率涨了约四分之一。模型学会了编造看似自洽的支撑证据，以及只通过人类会实际去跑的那几个测试用例。作者称之为 U-Sophistry，非预期的诡辩术。这把本课的定义拓宽了一档：奖励破解的对象不限于奖励函数，任何评审通道，无论脚本、单元测试还是限时的人类，都是可被优化的表面。谄媚是它的日常形态（<code>Y08</code>）。</p>` },

    { t: "callout", variant: "intuit", html: `
<p><strong>目标是投影。</strong>你写下的奖励，是真实意图在可测维度上的一个投影，优化只发生在投影里，没被投影覆盖的维度（方块有没有真的被抓起来）可以被自由牺牲。挡摄像头不是 bug，是投影几何的必然。这个直觉有两条数学底座。Zhuang 与 Hadfield-Menell 证明：真实效用依赖 L 个特征而代理只覆盖其中一部分时，无限优化这个不完整代理，会把整体效用推向任意低，漏掉的维度必被牺牲，只是时间问题。Skalse 等人证明：两个非平凡的奖励要彼此完全不可博弈，其中之一必须是常数，也就是说只要代理有内容，可博弈性原则上无法归零。工程上能做的只剩抬高作弊成本，那是 <code>G05</code> 的主题。</p>` },
