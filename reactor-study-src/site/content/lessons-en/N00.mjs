...
<p><strong>Correlated is not identical.</strong> The reason a ruler is worth using at all is that the dimension it measures correlates with what you actually care about: exam scores correlate with learning, rankings with quality, benchmark scores with whether a model is any good. Most of the failures collected here trace back to one thing: making the dial read higher usually has a route that is easier than doing the thing well. So the moment you start <em>rewarding</em> that correlated number, effort drifts from "do the thing well" to "make the dial read higher." Optimization pressure pulls on the correlation until it snaps.</p>` },
