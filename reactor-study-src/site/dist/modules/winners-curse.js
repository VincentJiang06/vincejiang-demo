...
import { mount, canvas, controls, slider, add, readout, legend, scoped, gauss, presets, annot } from "/modules/mod-kit.js?v=49b358d492";
...
mount("winners-curse", (body, fig, { config }) => {
  ...
  predictGate(body, {
    q: "先押一注：候选从 3 个加到 100 个，你选中的那个『赢家』，真实水平相对它的估计值会怎样？",
    options: [
      { t: "落差越拉越大：挑的动作专门捞『运气好的估计』，候选越多捞得越准", ok: true },
      { t: "落差不变：每个估计都是无偏的（平均不高估也不低估），挑多少个都一样" },
      { t: "落差缩小：候选池大了，总有真货冒头，选中的反而更可信" }
    ],
    reveal: () => {
      const rows = [[3, "0.85σ", toyGap(3).toFixed(2) + "σ"],
        [4, "103%", Math.round(toyGap(4) * 100) + "%"],
        [10, "154%", Math.round(toyGap(10) * 100) + "%"]];
      return `「取最大值」是个偏心的算子：估计里运气成分越大的候选，越容易排到最前面被你选中。单个估计无偏，不等于「按估计选出的最大者」无偏。为了对账，刚才用<strong>同一台模拟器</strong>重跑了课文里 Smith–Winkler 的玩具模型：把候选的真值全部设成 0（根本不存在更好的那个）、噪声 σ=1，各跑 2 万次，看选中者平均虚高多少：
      <table class="mini-table" style="margin-top:8px"><thead><tr><th>候选数</th><th>课文里的数字</th><th>本模拟器跑出</th></tr></thead>
      <tbody>${rows.map(r => `<tr><td>${r[0]} 个</td><td>${r[1]}</td><td>${r[2]}</td></tr>`).join("")}</tbody></table>
      <div class="label" style="margin-top:8px">上面主面板的 N 和 σ 拨得更大时，「失望缺口」按同一个机制继续放大。</div>`;
    },
    plain: "冠军的分数里永远掺着运气，候选越多、噪声越大，掺得越多。所以选完要打折（学名叫贝叶斯收缩）：不是惩罚冠军，是请它归还借来的运气——而且这一切发生时，没有任何人作弊。",
    actHint: "拨一拨候选数（或点上面三档），再对答案"
  }, buildSim);
  ...
  function buildSim(host, act) {
    ...
    const SCALES = [
      { k: "A", t: "3 个候选", n: 3, note: "三家外卖店里挑评分最高的那家" },
      { k: "B", t: "20 个候选", n: 20, note: "一轮面试里挑分数最高的候选人" },
      { k: "C", t: "100 个候选", n: 100, note: "一批 checkpoint 里挑 eval 最高分的那个" }
    ];
    let applying = false;
    const pb = presets(ctrl, {
      label: "一键换择优强度", items: SCALES,
      onselect: p => { applying = true; sN.input.value = p.n; sN.input.dispatchEvent(new Event("input")); applying = false; }
    });
    const sN = slider(host, { label: "候选数 N（择优强度）", min: 2, max: 200, step: 1, value: 20, fmt: v => v }).on(v => {
      N = v;
      if (!applying && pb.index != null) pb.select(null, false);   // 手动拖过就不再是那一档
      draw(); act();
    });
    const sS = slider(host, { label: "估计噪声 σ", min: 2, max: 30, step: 2, value: 10, fmt: v => (v / 10).toFixed(1) }).on(v => { noise = v / 10; draw(); act(); });
    add(ctrl, sN, sS);
    ...
    legend(host, [{ c: "var(--accent)", t: "选中者的估计值（你以为的）" }, { c: "var(--red)", t: "选中者的真实值（实际拿到的）" }]);
    ...
    function draw() {
      ...
      an.show([
        { kind: "box", x: xT, y: pad, w: Math.max(6, xE - xT), h: gh, t: `落差 ${(mE - mT).toFixed(2)}：选中者平均被高估这么多` },
        { x: xT, y: pad + 20, t: `实际拿到的：真值均线 ${mT.toFixed(2)}`, dir: "left" },
        { x: xE, y: pad + 48, t: `你以为的：估计均线 ${mE.toFixed(2)}`, dir: "left" }
      ]);
