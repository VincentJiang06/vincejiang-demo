...
import { mount, readout, scoped, presets, miniTable } from "/modules/mod-kit.js?v=49b358d492";

const VECTORS = [
  { k: "题目过拟合", plain: "背题", base: 34, blockedBy: ["heldout", "rotate"] },
  { k: "维度窄化", plain: "只练一面", base: 22, blockedBy: ["paired", "cap"] },
  { k: "直接造假", plain: "改数字", base: 18, blockedBy: ["audit", "decouple"] },
  { k: "风格/表面刷分", plain: "讨好评委", base: 16, blockedBy: ["paired", "audit"] }
];
const DEFENSES = [
...
const CASES = [
  { k: "00", t: "通用 eval（默认）", bases: [34, 22, 18, 16],
    line: "不指定场景的平均攻击面——原版沙盒就是它。" },
  { k: "01", t: "US News 法学院", bases: [22, 30, 28, 10],
    line: "冲分奖学金、就业统计口径任摆——「改数字」和「只练一面」是主通道（C02 打分：垄断+利害全高）。" },
  { k: "02", t: "商学院（五榜并存）", bases: [26, 22, 16, 14],
    line: "五家榜单互相稀释，单榜支配力掉档：四条通道都比法学院温和一点，但一条也没关上。" },
  { k: "03", t: "高考", bases: [46, 24, 8, 12],
    line: "题型可押、套路可练——「背题」一家独大；严格监考把「改数字」压到很低。" },
  { k: "04", t: "Chatbot Arena", bases: [28, 14, 10, 38],
    line: "长回复、emoji、讨喜语气——「讨好评委」是头号通道，外加适应榜口味的「背题」（私测择优另算，见 B13/Y11）。" },
  { k: "05", t: "好的内部诊断指标", bases: [10, 8, 4, 6],
    line: "不挂利害、没人有动机博弈：攻击面本来就小——这就是低反应性的样子（C02 打分全低）。" }
];
...
mount("design-robust-eval", (body, fig, { config }) => {
  ...
  intro.textContent = "勾选防御手段。系统会对你的 eval 发起一次 gaming 攻击，看还剩多少『幻觉』穿过去。目标：把真实信号占比顶上去。先用下面的案例换一套攻击面，再决定先堵哪条。";
  ...
  const pb = presets(body, {
    label: "一键载入 C02 打过分的案例（换一套初始攻击面）", items: CASES, value: 0,
    onselect: c => {
      bases = c.bases.slice();
      caseLine.textContent = "案例：" + c.t + " — " + c.line;
      feedback(`换到「${c.t}」：四条通道的压力重新分布了。看红色条找最粗的那条，先堵它——这也是 C02 说的「哪格高，失真先从哪格开始」。`);
      render();
    }
  });
  const caseLine = document.createElement("p");
  caseLine.className = "label"; caseLine.style.margin = "0 0 12px";
  caseLine.textContent = "案例：" + CASES[0].t + " — " + CASES[0].line;
  ...
  DEFENSES.forEach(([id, name, desc]) => {
    ...
    b.onclick = () => {
      ...
      const after = calc(on);
      const vecs = VECTORS.filter(v => v.blockedBy.includes(id)).map(v => `「${v.k}（${v.plain}）」`).join("和");
      if (on.has(id)) {
        feedback(after < before
          ? `勾上「${name}」：残余幻觉 ${before}% → ${after}%，一下挡掉 ${before - after} 个点——它堵的是 ${vecs}。`
          : `勾上「${name}」：读数没动，因为它管的 ${vecs} 已经被别的防御挡住了。这不是白勾——单层防御被绕穿时，它就是备份。多层组合的冗余正是把博弈成本顶上去的方式。`);
      } else {
        feedback(after > before
          ? `拆掉「${name}」：残余幻觉 ${before}% → ${after}%，回升 ${after - before} 个点——${vecs} 的通道又敞开了。缺一层，就漏一条道。`
          : `拆掉「${name}」：读数没动，${vecs} 还有别的防御顶着。但你刚拆掉的是那道备份。`);
      }
      render();
    };
    grid.appendChild(b);
  ...
  body.appendChild(fb);
  function feedback(msg) { fb.innerHTML = `<span class="label">刚才这一下</span><p>${msg}</p>`; }
  feedback("还没勾任何防御——四条通道全开着。先看下面哪条红色条最粗，挑一个专堵它的防御勾上，看读数怎么动。");
  ...
  const mt = miniTable(body, {
    cols: ["防御", "状态", "它堵的通道", "此刻的账"], rows: [],
    note: "逐防御对账（按边际算）：已开的=此刻拆掉它幻觉回升几个点；未开的=此刻勾上还能压几个点。0 点不等于没用——通道被别的防御挡着时，它是冗余备份。"
  });
  ...
  function render() {
    ...
    VECTORS.forEach((v, i) => {
      ...
      row.innerHTML = `<div style="display:flex;justify-content:space-between;font-size:.74rem;color:var(--text-2);font-family:var(--font-mono)"><span>${v.k}（${v.plain}）</span><span style="color:${blocked ? "var(--green)" : "var(--red)"}">${blocked ? "已挡下 " : ""}${remain}%</span></div>
    ...
    out.innerHTML = `残余幻觉 <span class="big" style="color:var(--red)">${illusion}%</span> &nbsp;·&nbsp; 真实信号占比 <span class="big" style="color:var(--green)">${real}%</span>
      <div style="font-size:.82rem;margin-top:6px;opacity:.9">${real >= 80 ? "✓ 稳了。注意：没有单一防御能挡住一切，是多层组合(尤其『解耦』+『held-out』)把博弈成本顶到不划算。" : on.size === 0 ? "裸奔的 eval：优化压力会把这几条通道全部走满。挑几个防御勾上。" : "还有通道敞着。看红色条：它们的 blockedBy 还没被你勾中。"}</div>`;

    mt.update(DEFENSES.map(([id, name]) => {
      const vecs = VECTORS.filter(v => v.blockedBy.includes(id)).map(v => v.plain).join("、");
      if (on.has(id)) {
        const without = new Set(on); without.delete(id);
        const d = calc(without) - calc(on);
        return [name, "已开", vecs, d > 0 ? `拆掉它幻觉 +${d} 点` : "冗余备份（0 点）"];
      }
      const withIt = new Set(on); withIt.add(id);
      const d = calc(on) - calc(withIt);
      return [name, "未开", vecs, d > 0 ? `勾上可再压 ${d} 点` : "此刻压不动（通道已被挡）"];
    }));
  }
  render();
});
