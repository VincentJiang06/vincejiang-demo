...
import { mount, canvas, controls, slider, add, readout, legend, scoped, stepper } from "/modules/mod-kit.js?v=4501323cdd";
...
mount("musiclab", (body, fig, { config }) => {
  ...
  intro.textContent = t("48 首歌，各有内在吸引力。听众顺序到达。拖动『社会影响』：看得到排行榜时，赢家通吃更极端，而谁封王变得几乎全看运气。下面的分步导览按当年真实实验的两轮走。");
  ...
  const sl = slider(body, { label: t("社会影响强度（榜单可见度）"), min: 0, max: 20, step: 2, value: 10, fmt: v => (v / 10).toFixed(1) }).on(v => { social = v / 10; run(); });
  add(ctrl, sl);
  ...
  legend(body, [{ c: "var(--accent)", t: t("各平行世界的市场份额（点）") }, { c: "var(--ink-300)", t: t("无社会影响时的平均（≈真实质量）") }]);
  let worlds;
  ...
  function draw() {
    ...
    ctx.fillStyle = C("--ink-400"); ctx.font = "10px monospace"; ctx.fillText(t("歌曲（按真实质量排序，左=最好）→"), pad + 4, h - 8);
    ...
    out.innerHTML = `${t("不平等 Gini")} <span class="big">${g.toFixed(2)}</span> &nbsp;·&nbsp; ${tf("最好那首歌在 {} 个世界的名次：", WORLDS)}<span class="big">${ranks.join(" ")}</span>
      <div style="font-size:.8rem;margin-top:6px;opacity:.9">${social > 0.6 ? t("看得到榜单：赢家通吃，且同一首歌的命运在平行世界间天差地别：早期噪声被放大成永久差距。") : t("没有社会影响：份额平缓、可预测，基本就是真实质量的样子。")}</div>`;
  }
  ...
  const STEPS = [
    { t: t("开场"), html: t(`这台模拟重演的是社会学里最著名的真人实验之一：MusicLab。点「下一幕」按当年实验的两轮走一遍（换幕会自动拨上面的滑块）；也可以随时直接拖滑块自己玩。`) },
    { t: t("第一轮（2006）· 独立世界"), v: 0, html: t(`14,341 名真人听 48 首无名乐队的歌。「独立世界」里谁也看不到别人的下载数，只凭耳朵——滑块已自动拨到 0。看图：各世界的点几乎贴着灰线（真实质量），Gini 低、名次稳。这条灰线就是每首歌的质量基准。`) },
    { t: t("第一轮（2006）· 社会影响世界"), v: 10, html: t(`另外 8 个平行世界能看到实时下载榜——滑块已拨到 1.0。两件事同时发生：Gini 升（更不平等），同一首歌在不同世界的名次天差地别（更不可预测）。信息更多、结果反而更难预测，说明榜单信号里混进了会被放大的早期噪声。作者的总结：最好的歌很少垫底，最差的歌很少夺冠，中间的一切皆有可能。`) },
    { t: t("第二轮（2008）· 倒置榜单"), v: 20, html: t(`续作《Leading the Herd Astray》（12,207 人）更狠：研究者直接造假，把真实最不受欢迎的歌显示为最热门。结果分两层——假流行短期真的自我实现了（R02 的实验室铁证）；但最好的那批歌长期恢复了人气，而且市场整体下载量下降：操纵榜单不只重排名次，还损耗整个市场。这个模拟没做倒置，滑块替你拉到了 2.0（最强羊群），感受同一机制的极端档：信号越强，早期运气滚得越大。`) }
  ];
  const renderStep = i => {
    const s = STEPS[i];
    if (s.v != null) setSocial(s.v);
    note.innerHTML = `<span class="label">${i ? tf("第 {}/3 幕 · ", i) : ""}${s.t}</span><p>${s.html}</p>`;
  };
  stepper(body, {
    count: STEPS.length, prevLabel: t("上一幕"), nextLabel: t("下一幕"),
    onstep: renderStep
  });
  renderStep(0);
