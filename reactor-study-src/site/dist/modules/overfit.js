...
import { mount, canvas, controls, slider, add, readout, legend, scoped, gauss } from "/modules/mod-kit.js?v=49b358d492";
...
mount("overfit", (body, fig, { config }) => {
  ...
  predictGate(body, {
    q: "先押一注：把「多项式次数」从 1 一路推到 11，训练误差和测试误差各会怎么走？",
    options: [
      { t: "训练误差一路降到底；测试误差先降后升", ok: true },
      { t: "两个都一路下降——模型越灵活，学得越好" },
      { t: "两个都先降后升——太复杂了连训练集都拟合不好" }
    ],
    reveal: `训练误差是模型能直接优化的分数，容量越大压得越低，永远单调向下；可你真正在乎的是新数据上的表现。过了某个点，模型不再学规律，开始背这批训练点特有的巧合和噪声——曲线扭着身子穿过每一个训练点、在点与点之间剧烈振荡，训练误差趋零、测试误差反弹。就像那个把往年真题背得滚瓜烂熟的学生：模拟考回回满分，新卷子当场垮掉。`,
    plain: "训练集就是你的 eval。在例题上的完美是可以买的，代价是新题上的真实——所以「eval 分数一路上涨」本身不说明变好，得有一批它从没见过的题来对质（这正是 held-out 测试集的意义）。",
    actHint: "把次数推到 8 以上、再拉回 2 以下，两头都看过就能对答案"
  }, buildSim);
  ...
  function buildSim(host, act) {
    ...
    add(ctrl, slider(host, { label: "多项式次数（模型容量）", min: 1, max: 11, step: 1, value: 3, fmt: v => v }).on(v => { deg = v; draw(); act(); }));
    ...
    legend(host, [{ c: "var(--ink-300)", t: "训练点" }, { c: "var(--accent)", t: "拟合曲线" }, { c: "var(--red)", t: "测试点" }]);
