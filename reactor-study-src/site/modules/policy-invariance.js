...
mount("policy-invariance", (body, fig, { config }) => {
  ...
  intro.textContent = t("历史数据里，通胀与失业有条漂亮的负相关(Phillips 曲线)。你想靠制造通胀来降失业。拖动『政策利用强度』，看人们的预期如何调整，把这条你据以定策的关系拉平、拉断。");
  body.appendChild(intro);
  let exploit = 0;
  const ctrl = controls(body);
  add(ctrl, slider(body, { label: t("政策利用强度（你多想用这条规律）"), min: 0, max: 100, step: 5, value: 0, fmt: v => v + "%" }).on(v => { exploit = v / 100; draw(); }));
  const { c, ctx, resize } = canvas(body, 300);
  const out = readout(body, "");
  legend(body, [{ c: "var(--ink-300)", t: t("历史观测（未被利用时）") }, { c: "var(--accent)", t: t("你利用之后的新数据") }]);
  ...
  function draw() {
    ...
    ctx.fillStyle = C("--ink-400"); ctx.font = "10px monospace"; ctx.fillText(t("失业 →"), pad + gw - 40, h - 8); ctx.save(); ctx.translate(12, pad + 40); ctx.rotate(-Math.PI / 2); ctx.fillText(t("通胀"), 0, 0); ctx.restore();
    ...
    out.innerHTML = `${t("新数据里 通胀↔失业 的相关")} <span class="big">${corr.toFixed(2)}</span>${t("（历史值 ≈ −0.9）")}
      <div style="font-size:.8rem;margin-top:6px;opacity:.9">${exploit > 0.4 ? t("关系崩了：一旦你系统性地制造通胀来买就业，人们把通胀<em>预期</em>进去，失业回到自然率，你只剩下高通胀。你用来定策的规律，因为你使用它而失效。这就是 Lucas 批判 = Goodhart 的宏观版。") : t("把政策利用强度推上去，看这条你想利用的负相关如何在你眼前拉直。")}</div>`;
  }
