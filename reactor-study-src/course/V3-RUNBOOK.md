...
## 3b. 文风契约 v3（2026-07-19 用户裁决，叠加于 v2 之上，Opus 4.8 全量检修执行）

v2 的「白话导语」被执行成了讲课体。修正方向：**亲和友善、易于理解，但绝无讲课感——
简单易懂、快速讲清理论的干练风**。具体刀法：
1. **去讲课化**：删除一切课程自指——「这节课/本课/上一课/这门课/接下来我们/为什么值得你花十分钟」
   之类的元话语全部清除。开篇直接进入现象或故事，不做「导游预告」。
2. **干练**：句子短、废话零。每段只做一件事。铺垫句(「值得注意的是」「有意思的是」「换句话说」连用)砍掉。
3. **保留亲和**：仍然说人话、仍然先例子后理论、术语首现仍给白话解释——亲和靠语言平实,不靠寒暄。
4. 结构件保留：callout 三型、「留一个问题」「一句话带走」、前置感知(非前置概念现场解释+链接)、
   全部事实/数字/考证不得增删。
5. 硬性字符规则照旧(无——/感叹号/正文反引号)。
